...
export async function fetchProfileFromSupabase(userId: string): Promise<LearnerProfile | null> {
  ...
  try {
    ...
    return {
      ...
      name: data.name,
      email: data.email,
      avatarSeed: data.avatar_seed || 'harron',
      ...
      overallMastery: data.overall_mastery ?? 0.7,
      overallRetention: data.overall_retention ?? 0.85,
      learningMomentum: data.learning_momentum ?? 80,
      calibrationScore: data.calibration_score ?? 85,
      ...
      accuracyRate: data.accuracy_rate ?? 0.8,
      averageResponseTimeSeconds: data.average_response_time_seconds ?? 12,
